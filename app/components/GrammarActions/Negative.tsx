...
import { NegativeSentences } from '@/app/action/prompt';
...
const Negative = () => {
    ...
    const handleNegative = useCallback(
        debounce(async (text) => {
            try {
                setIsLoading(true);
                CheckString(text);
                const response = await NegativeSentences({ text, action: `Convert between affirmative and negative form` });
    ...
    return (
        <Container>
            <h3 className='mt-3 text-lg '>Negative Sentences</h3>
            <p className='text-sm my-3'>Enter your sentence below to convert between affirmative and negative forms</p>
            ...
            >
                {isLoading ? 'Loading...' : 'Convert Sentence'}
            </button>
