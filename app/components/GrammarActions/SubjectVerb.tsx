...
import { SubjectVerbAgreement } from '@/app/action/prompt';
...
const SubjectVerb = () => {
    ...
    const handleSubjectVerb = useCallback(
        debounce(async (text) => {
            try {
                setIsLoading(true);
                CheckString(text);
                const response = await SubjectVerbAgreement({ text, action: `Check and correct subject-verb agreement` });
    ...
    return (
        <Container>
            <h3 className='mt-3 text-lg '>Subject and Verb Agreement</h3>
            <p className='text-sm my-3'>Enter your sentence below to check subject-verb agreement</p>
            ...
            <button
                ...
                disabled={isLoading}
                onClick={() => handleSubjectVerb(text)}
            >
                {isLoading ? 'Loading...' : 'Check Agreement'}
            </button>
